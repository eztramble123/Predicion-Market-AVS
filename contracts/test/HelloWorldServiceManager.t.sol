...
contract HelloWorldTaskManagerSetup is Test {
    ...
    // Define Option A and Option B based on the task's teams
    string memory optionA = "OptionA";
    string memory optionB = "OptionA";

    // Simulate randomness: Choose Option A or B randomly
    // Note: This is NOT secure for production use
    bool pickA = (uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty))) % 2) == 0;

    // Determine the winning team based on the random pick
    string memory winningOption = pickA ? optionA : optionB;

    // Create the message hash with the selected winning team
    bytes32 messageHash = keccak256(abi.encodePacked("Winning Option: ", winningOption));

    // Sign the message hash with the operator's signing key
    bytes memory signature = signWithSigningKey(operator, messageHash);

    // Prepare arrays for operators and signatures
    address[] memory operators = new address[](1);
    operators[0] = operator.key.addr;
    bytes[] memory signatures = new bytes[](1);
    signatures[0] = signature;

    // Encode the signed task data with the current block number
    bytes memory signedTask = abi.encode(operators, signatures, uint32(block.number));

    // Call the respondToTask method on the HelloWorldServiceManager contract
    ...
    // Optional: Emit an event for testing purposes
    emit TaskResponded(task.name, winningOption, operator.key.addr, signature);
}
}
